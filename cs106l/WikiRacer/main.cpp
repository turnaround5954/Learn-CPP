...
int numCommonLinks(const unordered_set<string>& curr_set, const unordered_set<string>& target_set) {
    return std::count_if(curr_set.begin(), curr_set.end(), [&target_set](const auto& it) {
        return target_set.find(it) == target_set.end() ? false : true;
    });
}
...
vector<string> findWikiLadder(const string& start_page, const string& end_page) {
    ...
    // Last exercise! please instantiate the priority queue for this algorithm, called "queue". Be
    // sure to use your work from Task 2, cmp_fn, to instantiate our queue. Estimated length: 1 line
