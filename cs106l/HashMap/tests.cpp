...
const std::vector<std::pair<std::string, int>> vec{
    {"A", 3}, {"B", 2},  {"C", 1},  {"A", -5},       {"B", 3},       {"A", 5},
    {"C", 1}, {"D", 10}, {"K", 30}, {"Avery", 2020}, {"Anna", 2020}, {"Ali", 2019}};
...
__attribute__((unused)) void unused_rehash_correct_by_time() {
    ...
    VERIFY_TRUE(
        ratio6_23 < (1 + tolerance) * 1500 / 500 && ratio6_23 > 1 / (1 + tolerance) * 1500 / 500,
        __LINE__);
    VERIFY_TRUE(
        ratio6_54 < (1 + tolerance) * 6000 / 1500 && ratio6_54 > 1 / (1 + tolerance) * 6000 / 1500,
        __LINE__);
    VERIFY_TRUE(
        ratio3_10 < (1 + tolerance) * 1600 / 500 && ratio3_10 > 1 / (1 + tolerance) * 1600 / 500,
        __LINE__);
    VERIFY_TRUE(
        ratio3_21 < (1 + tolerance) * 7500 / 1600 && ratio3_21 > 1 / (1 + tolerance) * 7500 / 1600,
        __LINE__);
    VERIFY_TRUE(
        ratio2_10 < (1 + tolerance) * 7500 / 3000 && ratio2_10 > 1 / (1 + tolerance) * 7500 / 3000,
        __LINE__);
...
void P_iter_algorithm_permutation() {
    ...
    VERIFY_TRUE(std::is_permutation(map1.begin(), map1.end(), answer.begin(), answer.end()),
                __LINE__);
    VERIFY_TRUE(std::is_permutation(answer.begin(), answer.end(), map1.begin(), map1.end()),
                __LINE__);
    VERIFY_TRUE(std::is_permutation(map2.begin(), map2.end(), answer.begin(), answer.end()),
                __LINE__);
    VERIFY_TRUE(std::is_permutation(answer.begin(), answer.end(), map2.begin(), map2.end()),
                __LINE__);
...
void Q_iter_const_iterator() {
    /* Tests the const-correct of your iterator class by asking for const_iterators via begin() and
     * end() */
...
void V_iter_erase_member() {
    /*
     * Verifies functionality of the erase and insert. Note that this erase test takes in a
     * parameter that is a const_iterator, different from test G which only takes in a key.
     */
    HashMap<std::string, int> map;
...
void A_range_ctor_basic() {
    ...
    std::unordered_map<std::string, int> answer{values.begin()++,
                                                values.end()};  // skips the first "Ignore me" pair
...
void C_initializer_list_ctor_basic() {
    ...
    std::unordered_map<std::string, int> answer{{"A", 3}, {"B", 2}, {"C", 1}, {"A", -5},
                                                {"B", 3}, {"A", 5}, {"C", 1}};

    HashMap<std::string, int> map{{"A", 3}, {"B", 2}, {"C", 1}, {"A", -5},
                                  {"B", 3}, {"A", 5}, {"C", 1}};
...
int A_benchmark_insert_erase() {
    ...
    for (size_t size : sizes) {
...

        {
            ...
            std::unordered_map<int, int, decltype(good_hash_function)> std_map(size,
                                                                               good_hash_function);
            for (int element : million) {
...
int run_test(const T& test, const string& test_name) {
    try {
        ...
        cout << "Test " << std::setw(30) << left << test_name << right << " FAIL with string " << e
             << '\n';
    } catch (const char*& e) {
        cout << "Test " << std::setw(30) << left << test_name << right << " FAIL with string " << e
             << '\n';
