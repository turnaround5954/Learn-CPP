 ...
 * ----------------------
 * This file implements the command-line controller for the Stanford 1-2-3
 * program.
 *
 ...
 * ----------------------------
 * The controller module implements the command-line interface to the
 * spreadsheet program.  The interface can easily be extended with additional
...
static void helpAction(TokenScanner & /* scanner */, SSModel & /* model */) {
    cout << left << setw(kLeftColumnWidth) << "help"
         << "Print this menu of commands" << endl;
    cout << left << setw(kLeftColumnWidth) << "load <filename>"
         << "Read named file into spreadsheet" << endl;
    cout << left << setw(kLeftColumnWidth) << "save <filename>"
         << "Save current spreadsheet to named file" << endl;
    cout << left << setw(kLeftColumnWidth) << "set <cell> = <value>"
         << "Set cell to value. Value can be \"string\" or formula" << endl;
    cout << left << setw(kLeftColumnWidth) << "get <cell>"
         << "Print contents of cell" << endl;
    cout << left << setw(kLeftColumnWidth) << "clear"
         << "Clear spreadsheet" << endl;
    cout << left << setw(kLeftColumnWidth) << "quit"
    ...
    while (true) {
        ...
        if (!cmdTable.containsKey(cmdName)) {
            ...
            try {
                ...
                cout << "Error in " << cmdName
                     << " command: " << ex.getMessage() << endl;
            }
        }
    }
}
