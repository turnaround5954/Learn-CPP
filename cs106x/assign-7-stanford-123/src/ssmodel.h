 ...
 * (i.e. controller/view) and thus need to be retained as is and implemented
 * correctly. However the rest of the class (both public & private) is yours to
 * design.
 */
...
class SSModel {
  public:
    ...
    double getCellValue(const std::string &name);

    // bool rangeIsValid(const location &st, const location &ed);
    bool rangeIsValid(const std::string &st, const std::string &ed);
    Vector<std::string> getCellInRange(const std::string &st,
                                       const std::string &ed);
  ...
  private:
    ...
    // store relations and properties
    Map<std::string, info *> lookup;

    // store relations
    // graph *rGraph;

    // helper functions
    bool detectCircle(const std::string &cellname,
                      Vector<std::string> dependencies);
    bool detectCircleHelper(const std::string &cellname,
                            const std::string &other,
                            Set<std::string> &visited);

    void updateSubGraph(const std::string &cellname);
    void updateSubGraphHelper(const std::string &cellname,
                              Set<std::string> &visited,
