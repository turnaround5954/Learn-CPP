...
typedef double (*rangeFunc)(const Vector<double> &values);
...
class Expression {
  ...
  public:
    static void parseDependencies(const Expression *exp,
                                  Vector<std::string> &dependencies);

    /**
     * Constructor: Expression
     * -----------------------
     * Specifies the constructor for the base Expression class.  Each
     * subclass constructor will define its own constructor.
     */
    ...
    virtual double eval(SSModel &context) const = 0;
...
class DoubleExp : public Expression {
  ...
  public:
    ...
    double eval(SSModel &context) const;
...
class CompoundExp : public Expression {
  ...
  public:
    ...
    CompoundExp(const std::string &op, const Expression *lhs,
                const Expression *rhs);

    /* Prototypes for the virtual methods overridden by this class */

    virtual ~CompoundExp();
    virtual double eval(SSModel &context) const;
 ...
 * Subclass: RangeExp
 * ---------------------
 * This subclass represents a range function expression.
 * A range function expression applies a named function to a cell range.
 */
class RangeExp : public Expression {
  public:
    RangeExp(const std::string &op, const std::string &st,
             const std::string &ed, SSModel &context);
    static bool containsFunc(std::string &op);
    virtual double eval(SSModel &context) const;
    virtual std::string toString() const;
    virtual ExpressionType getType() const;
    virtual Vector<std::string> getDependencies() const;

  private:
    static Map<std::string, rangeFunc> functionMap;
    std::string op, st, ed;
    Vector<std::string> dependencies;
};
