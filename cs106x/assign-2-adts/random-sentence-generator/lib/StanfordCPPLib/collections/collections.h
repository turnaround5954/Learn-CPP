 ...
 * @private
 */
std::ostream &writeQuotedString(std::ostream &os, const std::string &str, bool forceQuotes = true);
 ...
 * @private
 */
template <typename ValueType> std::ostream &writeGenericValue(std::ostream &os, const ValueType &value, bool) {
    return os << std::boolalpha << value;
}

inline std::ostream &writeGenericValue(std::ostream &os, char value, bool forceQuotes) {
    return writeQuotedChar(os, value, forceQuotes);
}

inline std::ostream &writeGenericValue(std::ostream &os, const std::string &value, bool forceQuotes) {
 ...
 * @private
 */
template <typename ValueType> bool readGenericValue(std::istream &is, ValueType &value) { return (bool)(is >> value); }
...
void checkVersion(const CollectionType &coll, const IteratorType &itr, const std::string &memberName = "") {
...
template <typename CollectionType, typename IteratorType>
void checkVersion(const CollectionType &, const IteratorType &, const std::string & = "") {
    // empty
}
#endif
 ...
 */
template <typename CollectionType> int compare(const CollectionType &coll1, const CollectionType &coll2) {
    // optimization: if they are the same object, then they are equal
    if (&coll1 == &coll2) {
        return 0;
    }

    auto itr1 = coll1.begin(), itr2 = coll2.begin(), end1 = coll1.end(), end2 = coll2.end();
 ...
 */
template <typename MapType> int compareMaps(const MapType &map1, const MapType &map2) {
    // optimization: if they are the same object, then they are equal
    if (&map1 == &map2) {
        return 0;
    }

    auto itr1 = map1.begin(), itr2 = map2.begin(), end1 = map1.end(), end2 = map2.end();
    for (; itr1 != end1 && itr2 != end2; ++itr1, ++itr2) {
        // compare each pair of elements from iterators
...
template <typename T, typename... Rest> int compareTo(const T &first, const T &second, const Rest &...rest) {
    if (first < second)
        return -1;
    if (second < first)
        return +1;
 ...
 * The element type must have an operator ==.
 */
template <typename CollectionType> bool equals(const CollectionType &coll1, const CollectionType &coll2) {
 ...
 */
template <typename CollectionType> bool equalsDouble(const CollectionType &coll1, const CollectionType &coll2) {
    // optimization: if literally same collection, stop
    if (&coll1 == &coll2) {
        return true;
    }
 ...
 * The key and value types must both have an operator ==.
 */
template <typename MapType> bool equalsMap(const MapType &map1, const MapType &map2) {
 ...
 */
template <typename IteratorType> int hashCodeIterable(IteratorType begin, IteratorType end, bool orderMatters = true) {
    int code = hashSeed();
    while (begin != end) {
        if (orderMatters) {
            code *= hashMultiplier();
 ...
 */
template <typename CollectionType> int hashCodeCollection(const CollectionType &collection, bool orderMatters = true) {
 ...
 */
template <typename MapType> int hashCodeMap(const MapType &map, bool orderMatters = true) {
    int code = hashSeed();
...
std::istream &readCollection(std::istream &input, CollectionType &collection, ElementType &element,
                             std::string descriptor = "readIterable") {
#else
template <typename CollectionType, typename ElementType>
std::istream &readCollection(std::istream &input, CollectionType &collection, ElementType &element,
...
std::istream &readMap(std::istream &input, MapType &map, KeyType &key, ValueType &value,
                      std::string descriptor = "readIterable") {
#else
template <typename MapType, typename KeyType, typename ValueType>
 ...
 * begin and end.
 */
template <typename IteratorType> std::ostream &writeIterable(std::ostream &out, IteratorType begin, IteratorType end) {
    out << "{";
    bool first = true;
    while (begin != end) {
        if (!first) {
 ...
 * begin and end.
 */
template <typename CollectionType> std::ostream &writeCollection(std::ostream &out, CollectionType collection) {
 ...
 * specially.
 */
template <typename MapType> std::ostream &writeMap(std::ostream &out, const MapType &map) {
    out << "{";
    auto begin = map.begin();
    auto end = map.end();
    bool first = true;
...
class VersionTracker {
  public:
     ...
     * object being moved.
     */
    VersionTracker(VersionTracker &&rhs) { rhs.mVersion++; }
...
template <typename Iterator> class CheckedIterator {
  public:
    ...
    CheckedIterator(const CheckedIterator<OtherItr> &rhs)
        : mVersion(rhs.mVersion), mOwner(rhs.mOwner), mIter(rhs.mIter), mBegin(rhs.mBegin), mEnd(rhs.mEnd) {}
    ...
    template <typename OtherItr> bool operator!=(const CheckedIterator<OtherItr> &rhs) const { return !(*this == rhs); }
    ...
    template <typename OtherItr> bool operator>(const CheckedIterator<OtherItr> &rhs) const { return rhs < *this; }
    template <typename OtherItr> bool operator<=(const CheckedIterator<OtherItr> &rhs) const { return !(*this > rhs); }
    template <typename OtherItr> bool operator>=(const CheckedIterator<OtherItr> &rhs) const { return !(*this < rhs); }
    ...
    CheckedIterator &operator+=(difference_type index) {
        if (!mOwner)
            error("Cannot advance uninitialized iterators.");
        ::stanfordcpplib::collections::checkVersion(*mOwner, *this);
        mIter += index;
        return *this;
    }
    CheckedIterator &operator-=(difference_type index) { return *this += (-index); }

    CheckedIterator operator+(difference_type index) const {
        auto result = *this;
        return result += index;
    }
    CheckedIterator operator-(difference_type index) const { return *this + (-index); }

    template <typename OtherItr> difference_type operator-(const CheckedIterator<OtherItr> &rhs) const {
        if (!mOwner || !rhs.mOwner) {
...
template <typename Iterator> class ProjectingIterator {
  public:
    ...
    /* Conversion constructor, when permitted. */
    template <typename OtherItr> ProjectingIterator(const ProjectingIterator<OtherItr> &rhs) : mIter(rhs.mIter) {
        // Empty
    }
    ...
    /* Comparison operators. */
    template <typename OtherItr> bool operator==(const ProjectingIterator<OtherItr> &rhs) { return mIter == rhs.mIter; }
    template <typename OtherItr> bool operator!=(const ProjectingIterator<OtherItr> &rhs) { return !(*this == rhs); }

    template <typename OtherItr> bool operator<(const ProjectingIterator<OtherItr> &rhs) { return mIter < rhs.mIter; }
    template <typename OtherItr> bool operator>(const ProjectingIterator<OtherItr> &rhs) { return rhs < *this; }
    template <typename OtherItr> bool operator<=(const ProjectingIterator<OtherItr> &rhs) { return !(*this > rhs); }
    template <typename OtherItr> bool operator>=(const ProjectingIterator<OtherItr> &rhs) { return !(*this < rhs); }

    /* Random access. */
    reference operator[](difference_type index) const { return mIter[index]; }
    ProjectingIterator &operator+=(difference_type index) {
        mIter += index;
        return *this;
    }
    ProjectingIterator &operator-=(difference_type index) { return *this += (-index); }
    ProjectingIterator operator+(difference_type index) const {
        auto result = *this;
        return result += index;
    }
    ProjectingIterator operator-(difference_type index) const { return *this + (-index); }
    template <typename OtherItr> difference_type operator-(const ProjectingIterator<OtherItr> &rhs) const {
        return mIter - rhs.mIter;
    }
    ...
    /* Dereferencing. */
    reference operator*() const { return mIter->first; }
    pointer operator->() const { return &**this; }
 ...
 */
template <typename SetTraits> class GenericSet {
  public:
    /**
     ...
     */
    template <typename... Args> explicit GenericSet(Args... args);
     ...
     * underlying map.
     */
    template <typename... Args> GenericSet(std::initializer_list<value_type> list, Args... args);
     ...
     */
    template <typename Traits> friend bool operator<(const GenericSet<Traits> &set1, const GenericSet<Traits> &set2);
    template <typename Traits> friend bool operator<=(const GenericSet<Traits> &set1, const GenericSet<Traits> &set2);
    template <typename Traits> friend bool operator>(const GenericSet<Traits> &set1, const GenericSet<Traits> &set2);
    template <typename Traits> friend bool operator>=(const GenericSet<Traits> &set1, const GenericSet<Traits> &set2);
  ...
  public:
    ...
    friend int hashCode(const GenericSet &set) { return hashCode(set.map); }
};

template <typename SetTraits>
GenericSet<SetTraits>::GenericSet(std::initializer_list<value_type> list) : map(SetTraits::construct()) {
...
template <typename SetTraits> void GenericSet<SetTraits>::add(const value_type &value) { map.put(value, true); }

template <typename SetTraits> GenericSet<SetTraits> &GenericSet<SetTraits>::addAll(const GenericSet &set2) {
...
template <typename SetTraits> typename GenericSet<SetTraits>::value_type GenericSet<SetTraits>::back() const {
    if (isEmpty()) {
...
template <typename SetTraits> void GenericSet<SetTraits>::clear() { map.clear(); }

template <typename SetTraits> bool GenericSet<SetTraits>::contains(const value_type &value) const {
    return map.containsKey(value);
}

template <typename SetTraits> bool GenericSet<SetTraits>::containsAll(const GenericSet &set2) const {
    for (const auto &value : set2) {
        if (!contains(value)) {
            return false;
        }
...
template <typename SetTraits> bool GenericSet<SetTraits>::equals(const GenericSet &set2) const {
...
template <typename SetTraits> typename GenericSet<SetTraits>::value_type GenericSet<SetTraits>::first() const {
    if (isEmpty()) {
...
template <typename SetTraits> typename GenericSet<SetTraits>::value_type GenericSet<SetTraits>::front() const {
    if (isEmpty()) {
...
template <typename SetTraits> void GenericSet<SetTraits>::insert(const value_type &value) { map.put(value, true); }
...
template <typename SetTraits> bool GenericSet<SetTraits>::isEmpty() const { return map.isEmpty(); }

template <typename SetTraits> bool GenericSet<SetTraits>::isSubsetOf(const GenericSet &set2) const {
...
template <typename SetTraits> bool GenericSet<SetTraits>::isSupersetOf(const GenericSet &set2) const {
    return containsAll(set2);
}

template <typename SetTraits> void GenericSet<SetTraits>::mapAll(std::function<void(const value_type &)> fn) const {
    map.mapAll([&](const value_type &elem, bool) { fn(elem); });
}

template <typename SetTraits> void GenericSet<SetTraits>::remove(const value_type &value) { map.remove(value); }

template <typename SetTraits> GenericSet<SetTraits> &GenericSet<SetTraits>::removeAll(const GenericSet &set2) {
    map.removeAll(set2.map);
    return *this;
}

template <typename SetTraits> GenericSet<SetTraits> &GenericSet<SetTraits>::retainAll(const GenericSet &set2) {
    map.retainAll(set2.map);
    return *this;
}

template <typename SetTraits> int GenericSet<SetTraits>::size() const { return map.size(); }

template <typename SetTraits> std::string GenericSet<SetTraits>::toString() const {
    std::ostringstream os;
    os << *this;
    return os.str();
}
 ...
 */
template <typename SetTraits> bool GenericSet<SetTraits>::operator==(const GenericSet &set2) const {
    return equals(set2);
}
...
template <typename SetTraits> bool operator<(const GenericSet<SetTraits> &set1, const GenericSet<SetTraits> &set2) {
...
template <typename SetTraits> GenericSet<SetTraits> GenericSet<SetTraits>::operator+(const GenericSet &set2) const {
    return GenericSet(*this).addAll(set2);
}

template <typename SetTraits> GenericSet<SetTraits> GenericSet<SetTraits>::operator+(const value_type &element) const {
...
template <typename SetTraits> GenericSet<SetTraits> &GenericSet<SetTraits>::operator+=(const GenericSet &set2) {
    removeFlag = false;
    return addAll(set2);
}

template <typename SetTraits> GenericSet<SetTraits> &GenericSet<SetTraits>::operator+=(const value_type &value) {
    add(value);
    removeFlag = false;
    return *this;
}
...
template <typename SetTraits> std::ostream &operator<<(std::ostream &os, const GenericSet<SetTraits> &set) {
    return stanfordcpplib::collections::writeCollection(os, set);
}

template <typename SetTraits> std::istream &operator>>(std::istream &is, GenericSet<SetTraits> &set) {
    typename SetTraits::ValueType element;
    return stanfordcpplib::collections::readCollection(is, set, element,
                                                       /* descriptor */ SetTraits::name() + "::operator >>");
}
 ...
 */
template <typename T> struct IsLessThanComparable {
  private:
    /* Use SFNIAE overloading to detect which of these two options to pick. */
    struct Yes {};
    struct No {};

    template <typename U> static Yes check(int, decltype(std::declval<U>() < std::declval<U>()) = 0);
    template <typename U> static No check(...);

  public:
    static constexpr bool value =
        std::conditional<std::is_same<decltype(check<T>(0)), Yes>::value, std::true_type, std::false_type>::type::value;
};

template <typename T> struct IsHashable {
  private:
    /* Use SFNIAE overloading to detect which of these two options to pick. */
    struct Yes {};
    struct No {};
    ...
    template <typename U>
    static Yes check(int, decltype(hashCode(std::declval<U>())) = 0,
 ...
 * more readable.
 */
template <typename T> std::function<bool(const T &, const T &)> checkedLess() {
    static_assert(IsLessThanComparable<T>::value, "Oops! You tried using a type as a key in our Map without making it "
                                                  "comparable. Click this error for more details.");
    /*
 ...
 */
template <typename... Args> struct Fail { static constexpr bool value = false; };
